...
<Info>
  [https://rxresume.org/schema.json](https://rxresume.org/schema.json)
</Info>
...
```json
{
  "$schema": "https://rxresume.org/schema.json",
  "version": "5.0.0"
}
```
...
const ajv = new Ajv();
const schema = await fetch("https://rxresume.org/schema.json").then((r) => r.json());
...
Below is the complete JSON Schema for Reactive Resume. You can also fetch the latest version directly from [https://rxresume.org/schema.json](https://rxresume.org/schema.json).
